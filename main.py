...
#############################################
	...
	x_dec1 = tf.contrib.nn.conv1d_transpose(x3_relu, ae_filters["deconv3"], [batch_size, size_of_data//2//latent_pool_size+1  ,10], stride=2, padding="SAME")
	x_dec1_relu = tf.nn.leaky_relu(x_dec1,alpha = 0.2)
	x_dec2 = tf.contrib.nn.conv1d_transpose(x_dec1_relu, ae_filters["deconv4"], [batch_size,2500,10], stride=latent_pool_size, padding="SAME")
...
def fit(x_dec3_relu, hm_epochs, patient, count=1):
	...
	while (abs(error[-1]-error[-2])>0.000000001 and N<hm_epochs):
		...
		for i in range(count):
			...
			_, c, error_inside, error_without_l2 = sess.run([optimizer, func_of_error,func_of_error_inside, func_of_error_without_l2],feed_dict={x_plac: epoch_x})
		...
		N = N + 1
	print(patient+' done')
	return (epoch_loss_current_inside, error_without_l2_on_epoch)

patients = [rd.choice( list(data.keys())) for i in range(5)] # take a random patients
for patient in patients:
	result_table = np.zeros((10,4))
	for param in [3,7]:
		for iteration in range(10):
			x_dec3_relu = set_architecture(param) 
			epoch_loss_current_inside, error_without_l2_on_epoch = fit(x_dec3_relu, 4000, patient)
			if(param == 3):
				result_table[iteration,0] = epoch_loss_current_inside
				result_table[iteration,1] = error_without_l2_on_epoch
			else:
				result_table[iteration,2] = epoch_loss_current_inside
				result_table[iteration,3] = error_without_l2_on_epoch

	np.savetxt(str(patient)+'.csv',result_table, delimiter = ';')
