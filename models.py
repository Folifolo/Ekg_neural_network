...
from utils import create_mask, data_generator, ECG_LEN, open_json


def build_model(latent_pool_size, x_ph):
...
def fit(model_output, mask_start, mask_end, epochs, patient, x_ph):
    x = tf.reshape(x_ph, [-1, ECG_LEN, 1])
    ...
    generator = data_generator(data, patient)
    ...
    for epoch in range(epochs):
        ...
        _, _, error_inside, error_outside = sess.run(
            [optimizer, error_function, error_function_inside, error_function_outside], feed_dict={x_ph: ecg})
